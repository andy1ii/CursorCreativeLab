...
// Dynamic colors
let bioPeakColor = '#d8f2ff'; 
let bioValleyColor = '#1e4fdb'; 
let bioScatterColor = '#6699e5'; 
...
const bioFragSource = `
  ...
  void main() {
      ...
      vec3 albedo = mix(u_valleyColor, u_peakColor, smoothstep(0.1, 0.8, h));
      float scatterIntensity = smoothstep(0.2, 0.5, h) - smoothstep(0.5, 0.8, h);
      albedo = mix(albedo, u_scatterColor, scatterIntensity * 0.5);

      vec3 finalColor = albedo * diffuse;
      finalColor += vec3(1.0) * specular; 
      finalColor += u_peakColor * fresnel * 0.6; 
...
function drawBio() {
  ...
  // Convert the hex strings into 0.0 - 1.0 RGB format safely for the shader
  let pc = color(bioPeakColor);
  let vc = color(bioValleyColor);
  let sc = color(bioScatterColor);
  
  bioRenderShader.setUniform('u_peakColor', [red(pc)/255.0, green(pc)/255.0, blue(pc)/255.0]);
  bioRenderShader.setUniform('u_valleyColor', [red(vc)/255.0, green(vc)/255.0, blue(vc)/255.0]);
  bioRenderShader.setUniform('u_scatterColor', [red(sc)/255.0, green(sc)/255.0, blue(sc)/255.0]);
